...
# AOSPA cerro: Lineage kernel flags (clang-stable, kernel-build-tools, no vendor/voltage).
include device/nubia/sm8650-common/build/BoardConfigKernel.mk
...
## Internal variables
COLLECT_MODULE_DEPS_CMD := python3 -c 'raise SystemExit("AOSPA cerro: TARGET_AUTO_COLLECT_KERNEL_MODULE_DEPS is not wired")'
...
ifneq ($(TARGET_KERNEL_CLANG_COMPILE),false)
    ...
    ifeq ($(KERNEL_CC),)
        ifneq ($(CCACHE_BIN),)
            KERNEL_CC := CC="$(CCACHE_BIN) $(TARGET_KERNEL_CLANG_PATH)/bin/clang" LD=$(TARGET_KERNEL_CLANG_PATH)/bin/ld.lld
        else
            KERNEL_CC := CC=$(TARGET_KERNEL_CLANG_PATH)/bin/clang LD=$(TARGET_KERNEL_CLANG_PATH)/bin/ld.lld
        endif
    endif
endif
...
ifeq ($(BOARD_USES_QCOM_MERGE_DTBS_SCRIPT),true)
	...
	PATH=$(abspath $(HOST_OUT_EXECUTABLES)):$${PATH} python3 $(BUILD_TOP)/device/nubia/sm8650-common/build/tools/merge_dtbs.py --base $(DTBS_BASE) --techpack $(DTB_OUT)/arch/$(KERNEL_ARCH)/boot/dts/vendor/ --out $(DTBS_OUT)
